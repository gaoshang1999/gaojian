        ...
        &lt;div class="form-group"> <br/> 
			 ...
			 @if($v->type == "date")
				 &lt;div class="input-group col-sm-4"> <br/> 
					&lt;input class="form-control date-picker col-xs-10 col-sm-5" id="{{$v->en }}" name="{{$v->en }}" type="text" data-date-format="yyyy-mm-dd" value="@{{ old('{!! $v->en!!}', ${!! $table->en!!}  ? ${!! $table->en!!}-> {!! $v->en!!} : '') }}"/> <br/> 
				...
				@elseif($v->type == "string")
					&lt;div class="col-sm-9"> <br/> 
						&lt;input type="text" id="{{$v->en }}" name="{{$v->en }}" placeholder="{{$v->cn}}" <br/> 
							class="col-xs-10 col-sm-5" value="@{{ old('{!! $v->en!!}', ${!! $table->en!!}  ? ${!! $table->en!!}-> {!! $v->en!!} : '') }}"/>  <br/> 
					&lt;/div> <br/> 
						
				@elseif($v->type == "text")
					&lt;div class="col-sm-9"> <br/> 
						&lt;textarea type="text/plain" id="{{$v->en }}"  name="{{$v->en }}" rows="5" class="col-xs-10 col-sm-5 autosize-transition">
						@{{ old('{!! $v->en!!}', ${!! $table->en!!}  ? ${!! $table->en!!}-> {!! $v->en!!} : '') }}&lt;/textarea> <br/> 
					&lt;/div> <br/> 	
					...
					&lt;div class="col-sm-9">						<br/> 
					    ...
					    &lt;option value="-1" >&lt;/option> <br/> 
					     <?php echo "@foreach"; ?>($constant as $c)  <br/> 
					        	&lt;option value="@{{ $c->k }}" <?php echo "@if"; ?>(${!! $table->en!!} && ${!! $table->en!!}->{!! $v->en!!} == $c->k ) selected <?php echo "@endif"; ?>  >@{{ $c->v }}&lt;/option> <br/> 
